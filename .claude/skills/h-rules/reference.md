...
> Sibling skill: `h-rules` — emits typed entries in `.gaia/rules.ts` from any skill's `reference.md` principles
> Paired with: `h-reference/reference.md` (5-part principle shape), `h-skill/reference.md` (skill conventions)
...
```
w-code  →  w-review (Gaia)  →  gstack /review  →  gstack /qa  →  gstack /ship  →  w-deploy
                                                                                       │
                                                                                       └─ on failure → w-debug
```

`w-review` is fast pass/fail against `rules.ts`; gstack `/review` is deep correctness; `/qa` runs tests + e2e; `/ship` merges; `w-deploy` deploys. A failure at any gate kicks back to coding.
...
// .claude/skills/a-security/reference.md describes "Protected by default"
// .gaia/rules.ts:
{ id: 'security/protected-by-default', skill: 'a-security',
  tier: 'ast-grep',
...
- Pending → enforced cycle time SLO: 14 days; older entries surface in `a-health`.
...
References load via the `skill-reference` hook on Skill invocation, plus the `domain-context` hook on file edit (walks the folder tree). Skills run via the `Skill` tool, with ≥3 phases per `h-skill/reference.md`. Skills routinely `Read` references mid-execution. Both have a `Last verified:` field — staleness is debt.
...
- `Last verified:` field mandatory; >180 days is debt picked up by `a-health`.
...
See `.claude/skills/w-review/SKILL.md` — frontmatter, phases, pre-condition + final-gate sandwich, output format, with `reference.md` sibling carrying the principles.
...
- Quarterly skill audit in `a-health`; unused skills (no invocation in 6 months) get archived.
...
`.claude/skills/w-review/{SKILL.md,reference.md}` — full pair.
...
- Skill references = principles for an authoring or audit task (e.g. `a-security/reference.md`).
...
The agent writes to `memory/episodic/` during sessions. After 90 days without re-trigger, entries archive. Promotion to a skill's `reference.md` is manual via `h-rules` and requires the same review as a code change. `memory/personal/` is gitignored at the directory level — never commit personal notes.
...
- `h-rules` is the only skill that promotes; code review enforces.
...
```sh
# ✅ Episodic learning gets observed → curated → promoted (via h-rules)
echo "Pattern observed: ..." >> .gaia/memory/episodic/$(date +%F).md
# 90 days later, h-rules reviews; if pattern is generalizable, promote to a reference principle + rules.ts entry
```
...
The `h-rules` skill (or human review) sees a pattern in `memory/episodic/` ≥3 times across distinct contexts → proposes a reference addition. Promotion requires:
...
- "Use AppError, not bare Error" (`w-code/reference.md`) → `code/named-errors-no-bare-throw` ast-grep rule
- "Don't read `process.env` outside config/env.ts" (`a-security/reference.md`) → `security/no-raw-env` hook
- "Tests next to source" (`w-code/reference.md`) → `testing/colocated-tests` script
...
- Reference shape: `.claude/skills/h-reference/reference.md`
