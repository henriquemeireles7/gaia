---
name: a-security
description: 'Security audit skill. Reads codebase against the security principles, surfaces findings, optional auto-fix or report-only. Mode: report (default) | fix (with explicit confirmation). Tier: daily (zero-noise smoke) | comprehensive (full OWASP + STRIDE + supply chain). Triggers: "a-security", "security audit", "audit security". Pair: w-review (run before merging anything touching auth, secrets, input validation). Artifact: .gaia/audits/a-security/<YYYY-MM-DD>.md.'
---

# a-security — Security audit
...
- `/a-security` — full audit, fix mode (auto-fix where mechanical).
- `/a-security report` — report-only, no mutations.
- `/a-security <scope>` — narrow to a single principle or path glob.
