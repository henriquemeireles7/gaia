...
This shape is how `h-reference` skill writes new references. It is also what `w-review` checks during reference reviews.
...
**Enforcement:** `w-review` voice/length check (planned `scripts/check-reference-shape.ts`); rule `references/voice-imperative` (pending — soft regex).
...
Title + description → 2–4 rules → enforcement → anti-pattern → pattern. No exceptions. The shape is what makes references mechanically reviewable, what makes `h-reference` writable, and what gives the agent a predictable place to find each part.
...
**Enforcement:** Encoded in `h-reference` skill (`.claude/skills/h-reference/SKILL.md`). The skill IS the review process; reviewer reads the skill output before approving the PR.
...
References rot silently. The `Last verified` field in the header makes rot visible. The `a-health` quarterly audit picks up stale references and surfaces them; either the reference is re-verified (date bumps) or it's revised.

**Rules / Guidelines / Boundaries:**

- Header MUST include `> Last verified: <Month YYYY>`
- 180-day window: anything older is "stale" debt
- `a-health` audit runs quarterly and lists stale references
...
This is what `h-reference` skill executes step-by-step.
...
| ------------------------------------------ | -------------------------------------------- | ------------------------------------- |
| 1. Consulted, not taught                   | `w-review` voice check (planned)             | _pending: references/voice-consulted_ |
...
| 6. Adversarial review                      | `h-reference` skill                          | `references/adversarial-review`       |
...
| 2026-04-28 | Adversarial review (6 specialists) before any new reference       | Prevents single-author blind spots; codified in `h-reference` skill so the workflow is reproducible.                                |
