...
- `/d-rules audit` — walk every skill, surface unmapped principles + pending entries past 14-day SLO.

## What this does

For each principle in a skill's `reference.md` (5-part shape per `d-reference/reference.md`), emit a typed entry in `.gaia/rules.ts` with mechanism + tier + reference link. Closes the Skill ↔ Reference ↔ Rules triad.
...
2. The skill has a `reference.md` sibling. If missing, abort and tell the operator to run `d-reference` first.
3. The reference uses the 5-part principle shape (per `d-reference/reference.md`). If it doesn't, abort and tell the operator to run `d-reference` first.
...
- **id candidate** — `<skill-domain>/<kebab-title>` (e.g. `security/protect-config` for the principle "Protect config files" in `d-security/reference.md`).
