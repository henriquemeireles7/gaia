---
name: w-initiative
description: "Strategy session skill. Interactive founder Q&A that produces an initiative document with project suggestions. Mode: fix. Tier: scoping (single-bet sketch) | full-bet (research + strategy + PR breakdown). Triggers: 'w-initiative', 'strategy session', 'new initiative', 'what should we build next'. After: (entry point of the workflow loop). Artifact: .gaia/initiatives/NNNN-<slug>/initiative.md."
---
...
**w-initiative** → gstack reviews (CEO/eng/design) → w-initiative → /ship
...
- **Founder Q&A produces incoherent input** (contradictions, no falsifier, no hypothesis) — escalate to `decisions/humantasks.md` with the unresolved questions. Don't fabricate strategy from thin context.
- **Continuity check finds the new initiative duplicates an in-flight one** — pause, surface the conflict, ask whether to merge or supersede.
- **Validate-artifacts script fails on the new initiative** (missing parent, hypothesis, falsifier, measurement) — revert the file write; report the missing fields.
