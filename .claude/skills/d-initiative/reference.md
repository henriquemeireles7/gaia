...
> Status: scaffolded — full 5-part principle shape lands when `d-initiative` runs in expansion mode (post-0001).
> Sibling skill: `d-initiative` (this folder's `SKILL.md`).
...
```
d-initiative (Q&A → writes initiative.md with all 5 sections)
   ↓
gstack /autoplan (reviews end-to-end, appends AD-N to §5)
   ↓
d-code (reads §4, codes PR by PR)
```

## Cross-references

- Sibling skill: `.claude/skills/d-initiative/SKILL.md`
- Workflow loop: `.claude/skills/d-rules/reference.md` Part 4
