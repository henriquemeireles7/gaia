---
name: w-code
description: "TDD implementation from project roadmaps. Five phases: plan decomposition, TDD implementation, completion audit, wiring audit, handoff. Mode: fix. Triggers: 'w-code', 'implement', 'start coding', 'code this project'. After: w-initiative. Artifact: commits + PR per project entry in the initiative PR breakdown."
---

# w-code — TDD Implementation
...
w-initiative → gstack reviews → w-initiative → **w-code** → w-review → /ship
...
- When done, ALWAYS suggest `/w-review` before `/ship`

## Failure modes

- **TDD red→green can't be achieved within reasonable iterations** — escalate to `decisions/humantasks.md` with the failing test and the attempted fixes. Don't disable the test to ship.
- **`bun run check` red on the final gate** — back out the change set; report which check failed (lint, typecheck, test) and why.
- **Wiring audit finds the change isn't reachable from the live UI/API** — flag explicitly; don't claim "done" if dead code shipped.
- **Initiative PR breakdown lists work outside the project's domain** — pause and ask the operator whether to expand scope or defer.
