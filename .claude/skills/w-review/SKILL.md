---
name: w-review
description: "Intelligent pre-commit review: script-first mechanical checks + AI-powered logic review. Fix-first, finding-first. Mode: fix. Tier: pre-commit | pre-merge | pre-release. Triggers: 'w-review', 'check quality', 'review the code', 'pre-commit check', 'gaia review'. Voice: 'review my code', 'check my work', 'pre-commit review'. After: w-code or w-debug. Artifact: PR comment + commit if auto-fix applied."
---
...
- `/w-review` — Full review: orchestrator + 6 AI phases (~1-2 min)
- `/w-review quick` — Orchestrator + final gate only (~15s)
- `/w-review mechanical` — Just orchestrator JSON, no AI phases
...
w-code → **w-review** → /ship

## When to run

- Before every commit
- Before /ship
- After w-code completes a project
...
```sh
bun .claude/skills/w-review/scripts/review-orchestrator.ts
```
...
**For `/w-review quick`:** Skip to Phase 7 after this phase.
**For `/w-review mechanical`:** Output the JSON and stop.
...
- On the next `/w-review` invocation, check for an incomplete prior review
...
Mode: **fix** — w-review is fix-first: applies mechanical fixes immediately and surfaces remaining findings sorted by confidence. Reports issues with numeric confidence scores per finding.
...
- NEVER skip a phase (use `/w-review quick` if you need speed)
...
- Coherence checks only apply to entities touched by the diff (that's a-health's job for the full codebase)
- Lead the report with findings, not phases

## Failure modes

- **AI review subagent fails or times out** — degrade to script-only mode (mechanical checks). Report which AI phases ran vs skipped.
- **Auto-fix introduces a check failure** (auto-fixed code triggers a different rule) — revert the auto-fix and surface the conflict for human review. Don't loop on auto-fix retries.
- **No diff to review** (clean tree) — short-circuit with a one-line report. Don't fabricate findings.
- **Pre-merge tier finds critical issues** (security, data loss, broken contract) — block the merge; escalate to `decisions/humantasks.md` rather than auto-resolve.
