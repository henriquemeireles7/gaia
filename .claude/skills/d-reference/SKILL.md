---
name: d-reference
description: 'Author or rewrite a .gaia/reference/*.md file. Adversarial-review per principle, then 5-part shape rewrite, then add rules.ts entries. Triggers: "d-reference", "write a reference", "rewrite reference", "create new reference", "audit reference".'
---

# d-reference — Author / rewrite a reference file

## Quick Reference

- `/d-reference <name>` — author a new reference at `.gaia/reference/<name>.md` (or `product/<name>.md` / `features/<name>.md`)
- `/d-reference rewrite <path>` — adversarial rewrite of an existing reference
