...
export const rules: readonly Rule[] = [
  ...
  // ─── w-initiative/reference.md (8 principles, pending enforcement) ──
  {
    ...
    mechanism: {
      kind: 'pending',
      note: 'scripts/validate-artifacts.ts (planned) — assert frontmatter `falsifier:` field present and non-empty',
    },
  },
  {
    ...
    mechanism: {
      kind: 'pending',
      note: 'scripts/validate-artifacts.ts (planned) — assert frontmatter `wave:` field present and one of {0a,0b,1,2,3,4,5,off-wave}',
    },
  },
  {
    ...
    mechanism: {
      kind: 'pending',
      note: 'scripts/validate-artifacts.ts (planned) — assert §2 contains a `| Surface | Ships v1.0 | Capped |` table with ≥3 rows',
    },
  },
  {
    ...
    mechanism: {
      kind: 'pending',
      note: 'scripts/validate-artifacts.ts (planned) — assert §2 contains a `| Trigger | Next step |` table with ≥1 row',
    },
  },
  {
    ...
    mechanism: {
      kind: 'pending',
      note: 'scripts/validate-artifacts.ts (planned) — assert §6 table has a Class column with values in {M,T,UC}',
    },
  },
  {
    ...
    mechanism: {
      kind: 'pending',
      note: 'scripts/validate-artifacts.ts (planned) — assert frontmatter `measurement.query` is non-empty and not literal "TBD" past `draft` status',
    },
  },
  {
    ...
    mechanism: {
      kind: 'pending',
      note: 'scripts/validate-artifacts.ts (planned) — assert §1.5 contains a `Panel:` heading with ≥6 specialist rows',
    },
  },
  {
    ...
    mechanism: {
      kind: 'pending',
      note: 'scripts/validate-artifacts.ts (planned) — assert §1 contains a `Sources read` heading with ≥3 path entries',
    },
  },
