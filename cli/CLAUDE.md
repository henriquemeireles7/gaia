...
The Gaia CLI — published on npm as `create-gaia-app` with two bins: `create-gaia-app` (the scaffolder, invoked by `bun create gaia-app@latest <name>`) and `gaia` (the in-project verb runner, invoked by `bun gaia <verb>`). This is the front door of initiative 0002: a developer runs `bun create gaia-app@latest <name>` and the scaffolder takes them from `git clone` to live URL in ≤30 minutes.

## Folder layout

| Path                    | Purpose                                                                                                                     |
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| `package.json`          | Standalone npm package, published independently. `bin` exposes `gaia` (verb dispatcher) and `create-gaia-app` (scaffolder). |
...
- **Standalone publishable.** This package MUST work installed alone via `bun add -g create-gaia-app` (or `npm i -g create-gaia-app`) with no monorepo context. Do NOT import from `@gaia/*` workspace packages — copy the code or duplicate the type. The CLI has its own minimal dependency tree.
...
- **TTHW-1 < 1000ms.** Banner must appear in <1s from `bun create gaia-app@latest` exec. Benchmark gate in `cli/test/banner.test.ts`.
