...
import { rules, type SkillDomain } from '../.gaia/rules'
...
function skillFor(file: string): SkillDomain | null {
  // .claude/skills/<skill>/reference.md → <skill>
  const skillMatch = file.match(/^\.claude\/skills\/(d-[a-z-]+)\/reference\.md$/)
  ...
  // .gaia/reference/product/<x>.md → product/<x>
  const productMatch = file.match(/^\.gaia\/reference\/product\/([a-z-]+)\.md$/)
  if (productMatch) return `product/${productMatch[1]}` as SkillDomain
...
const SCAN_PATTERNS = [
  '.claude/skills/d-*/reference.md',
  'apps/*/CLAUDE.md',
  'packages/*/CLAUDE.md',
  '.gaia/reference/product/*.md',
]
